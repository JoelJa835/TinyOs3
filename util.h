...
typedef struct connection_request_type connection_request;/**< @brief Forward declaration */
...
typedef struct resource_list_node {
  ...
  /** @brief The list node's key.
     ...
     */
  union {
  	PTCB* ptcb; /** @brief Added support for ptcb.*/
    PCB* pcb; 
    TCB* tcb;
    CCB* ccb;
    DCB* dcb;
    FCB* fcb;
    connection_request* con_req; /** @brief Added support for connection requests.*/
    void* obj;
    rlnode_ptr node;
    intptr_t num;
    uintptr_t unum;
